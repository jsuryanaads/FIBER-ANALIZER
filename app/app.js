...
$("addAsset")?.addEventListener("click",openAsset);$("addCable")?.addEventListener("click",e=>{e.preventDefault();if(!requirePermission("network.write"))return;if(!(db.assets||[]).length){alert("Buat Node/Asset terlebih dahulu sebelum menambahkan kabel.");return}openCable();});$("cableFrom").onchange=syncCablePorts;$("cableTo").onchange=syncCablePorts;$("search").oninput=renderAssets;
$("assetForm").onsubmit=e=>{e.preventDefault();const id=$("assetId").value||crypto.randomUUID(),type=$("assetType").value,existing=db.assets.find(a=>a.id===id),portCount=type==="ODP"?8:Math.max(1,Number($("assetPortCount").value||1));const item={id,type,code:$("assetCode").value.trim(),name:$("assetName").value.trim(),status:$("assetStatus").value,port_count:portCount};if(type==="OLT"){item.olt_ports=Array.from({length:portCount},(_,i)=>existing?.olt_ports?.[i]||({port_number:i+1,code:item.code+"-P"+(i+1),status:"AVAILABLE"}))}if(!item.code||!item.name){alert("Kode dan nama asset wajib diisi.");return}const i=db.assets.findIndex(a=>a.id===id);if(i>=0)db.assets[i]=item;else db.assets.push(item);save();$("assetDialog").close();render()};
...
$("cableForm").onsubmit=e=>{e.preventDefault();if(!requirePermission("network.write"))return;const id=$("cableId").value||crypto.randomUUID(),from=$("cableFrom").value,to=$("cableTo").value,count=Math.max(1,Number($("cableFiberCount").value||1)),code=$("cableCode").value.trim(),fromPort=Math.max(1,Number($("cableFromPort").value||1)),toPort=Math.max(1,Number($("cableToPort").value||1)),length=Number($("cableLength").value||0);if(!from||!to||from===to){alert("Node asal dan tujuan harus berbeda.");return}if(!code){alert("Kode kabel wajib diisi.");return}if(!Number.isFinite(length)||length<0){alert("Panjang kabel tidak valid.");return}const fromNode=db.assets.find(a=>a.id===from),toNode=db.assets.find(a=>a.id===to);const portCount=a=>Math.max(1,Number(a?.port_count)||(a?.type==="OLT"?16:1));if(fromPort>portCount(fromNode)||toPort>portCount(toNode)){alert("Port kabel melebihi jumlah port node yang dipilih.");return}const duplicate=db.cables.find(c=>c.id!==id&&String(c.code||"").trim().toLowerCase()===code.toLowerCase());if(duplicate){alert("Kode kabel sudah digunakan: "+duplicate.code);return}const item={id,code,fiber_count:count,length_m:length,from,to,fromPort,toPort,status:$("cableStatus").value};const i=db.cables.findIndex(c=>c.id===id);if(i>=0){db.cables[i]=item;db.coreConnections=(db.coreConnections||[]).filter(x=>x.inputCableId!==id&&x.outputCableId!==id);db.cores=db.cores.filter(c=>c.cable_id!==id)}else db.cables.push(item);for(let n=1;n<=count;n++)db.cores.push({id:crypto.randomUUID(),cable_id:id,core_number:n,status:"AVAILABLE"});save();$("cableDialog").close();render()};
