...
function renderCustomerPage(){
  ...
  el.innerHTML=rows.map(x=>'<div class="cable-card"><div><b>'+esc(x.code)+'</b><span class="badge">'+esc(x.status||"ACTIVE")+'</span></div><div class="muted">'+esc(x.name)+' · Customer</div><div class="cable-actions"><button data-page-link="trace-analysis" data-page-hash="" data-customer-context="'+esc(x.id)+'">Trace</button><button data-page-link="optical-analyzer" data-customer-context="'+esc(x.id)+'">Optical</button>'+(roleCan("customer.write")?'<button data-customer-edit="'+esc(x.id)+'">Edit</button><button class="danger" data-customer-del="'+esc(x.id)+'">Hapus</button>':"")+'</div></div>').join("")||'<p class="muted">Belum ada customer.</p>';
  el.querySelectorAll("[data-customer-context]").forEach(btn=>btn.onclick=e=>{
    e.preventDefault();
    navigateContext(btn.dataset.pageLink,{customer:btn.dataset.customerContext});
  });
}
...
function renderOltPorts(){const el=$("oltPorts");if(!el)return;const olts=db.assets.filter(a=>a.type==="OLT");const cables=db.cables||[];const used=new Map();for(const c of cables){if(c.from)used.set(c.from+"|"+c.fromPort,c);if(c.to)used.set(c.to+"|"+c.toPort,c)}el.innerHTML=olts.map(olt=>'<div class="olt-port-group"><div class="panel-head"><div><h3>'+esc(olt.code)+' — '+esc(olt.name)+'</h3><span class="muted">'+esc(olt.port_count||0)+' port</span></div></div><div class="port-grid">'+Array.from({length:Math.max(1,Number(olt.port_count)||1)},(_,i)=>{const n=i+1,p=olt.olt_ports?.[i]||{port_number:n,code:olt.code+"-P"+n,status:"AVAILABLE"},c=used.get(olt.id+"|"+n);return '<div class="port-card '+(c?"used":"available")+'"><b>Port '+n+'</b><span>'+esc(p.code)+'</span><small>'+esc(c?c.code:"AVAILABLE")+'</small></div>'}).join("")+'</div></div>').join("")||'<p class="muted">Belum ada OLT.</p>'}
function renderStats(){const el=$("stats");if(!el)return;const counts=Object.fromEntries(topology.map(t=>[t,db.assets.filter(a=>a.type===t).length]));el.innerHTML=topology.map(t=>'<div class="stat"><b>'+counts[t]+'</b><span>'+typeLabel(t)+'</span></div>').join("")+'<div class="stat"><b>'+db.cores.length+'</b><span>CORES</span></div>'}
...
function renderCondition(){
  const coreEl=$("coreCondition"),serviceEl=$("serviceStatus");
  if(!coreEl&&!serviceEl)return;
  ...
  if(serviceEl)$("serviceStatus").innerHTML='<div class="service"><span>🟢 Pelanggan Aktif</span><b>'+active+'</b></div><div class="service"><span>🔴 Pelanggan Nonaktif</span><b>'+(customers.length-active)+'</b></div><div class="service"><span>⚠ Gangguan Aktif</span><b>'+openIncidents+'</b></div><div class="service"><span>🔧 Work Order Open</span><b>'+openWorkOrders+'</b></div>';
}
function renderAssets(){const searchEl=$("search"),assetsEl=$("assets");if(!searchEl||!assetsEl)return;const q=searchEl.value.toLowerCase();const a=db.assets.filter(x=>(x.code+" "+x.name+" "+x.type).toLowerCase().includes(q));assetsEl.innerHTML=a.map(x=>'<div class="row"><div><b>'+esc(x.code)+'</b><div class="muted">'+esc(typeLabel(x.type))+' · '+esc(x.name)+'</div></div><span class="badge">'+esc(x.status)+'</span><div><button data-edit="'+esc(x.id)+'">Edit</button> <button class="danger" data-del="'+esc(x.id)+'">Hapus</button></div></div>').join("")||'<p class="muted">Tidak ada asset.</p>'}
function renderCustomers(){const customerEl=$("customerSelect"),optEl=$("optCustomer");if(!customerEl&&!optEl)return;const html=db.assets.filter(x=>x.type==="CUSTOMER").map(x=>'<option value="'+esc(x.id)+'">'+esc(x.code)+' — '+esc(x.name)+'</option>').join("");if(customerEl)customerEl.innerHTML=html;if(optEl)optEl.innerHTML='<option value="">— Pilih customer route —</option>'+html}
function renderCores(){const el=$("cores");if(!el)return;const cables=Object.fromEntries(db.cables.map(c=>[c.id,c]));el.innerHTML='<table class="table"><thead><tr><th>Cable</th><th>Core</th><th>Status</th><th>Length</th></tr></thead><tbody>'+db.cores.map(c=>'<tr><td>'+esc(cables[c.cable_id]?.code||"-")+'</td><td>'+c.core_number+'</td><td><span class="badge">'+esc(c.status)+'</span></td><td>'+esc(cables[c.cable_id]?.length_m||"-")+' m</td></tr>').join("")+'</tbody></table>'}
...
$("addSplitter")?.addEventListener("click",()=>openSplitter());
$("splitterNode")?.addEventListener("change",()=>{
...
$("splitterConnectionNode")?.addEventListener("change",()=>{
...
$("splitterConnectionForm")?.addEventListener("submit",e=>{
  e.preventDefault();
...
$("splitterConnectionList")?.addEventListener("click",e=>{
...
$("addSplitterConnection")?.addEventListener("click",()=>openSplitterConnection());
$("splitterList")?.addEventListener("click",e=>{
...
$("optCustomer")?.addEventListener("change",()=>{ $("customerSelect").value=$("optCustomer").value; renderOpticalAnalyzer(); };
$("customerSelect")?.addEventListener("change",()=>{ $("optCustomer").value=$("customerSelect").value; renderOpticalAnalyzer(); };
["optWavelength","optAttenuation","optConnectorCount","optConnectorLoss","optSpliceCount","optSpliceLoss","optMargin","optBudget"].forEach(id=>$(id)?.addEventListener("input",renderOpticalAnalyzer));
$("optWavelength")?.addEventListener("change",()=>{ const defaults={1310:"0.35",1490:"0.30",1550:"0.22"}; if($("optAttenuation").value===""||Number($("optAttenuation").value)===0)$("optAttenuation").value=defaults[$("optWavelength").value]||"0.35"; renderOpticalAnalyzer(); };
