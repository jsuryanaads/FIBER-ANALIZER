...
function renderOltPorts(){const olts=db.assets.filter(a=>a.type==="OLT");const cables=db.cables||[];const used=new Map();for(const c of cables){if(c.from)used.set(c.from+"|"+c.fromPort,c);if(c.to)used.set(c.to+"|"+c.toPort,c)}el.innerHTML=olts.map(olt=>'<div class="olt-port-group"><div class="panel-head"><div><h3>'+esc(olt.code)+' — '+esc(olt.name)+'</h3><span class="muted">'+esc(olt.port_count||0)+' port</span></div></div><div class="port-grid">'+Array.from({length:Math.max(1,Number(olt.port_count)||1)},(_,i)=>{const n=i+1,p=olt.olt_ports?.[i]||{port_number:n,code:olt.code+"-P"+n,status:"AVAILABLE"},c=used.get(olt.id+"|"+n);return '<div class="port-card '+(c?"used":"available")+'"><b>Port '+n+'</b><span>'+esc(p.code)+'</span><small>'+esc(c?c.code:"AVAILABLE")+'</small></div>'}).join("")+'</div></div>').join("")||'<p class="muted">Belum ada OLT.</p>'}
function renderStats(){const counts=Object.fromEntries(topology.map(t=>[t,db.assets.filter(a=>a.type===t).length]));el.innerHTML=topology.map(t=>'<div class="stat"><b>'+counts[t]+'</b><span>'+typeLabel(t)+'</span></div>').join("")+'<div class="stat"><b>'+db.cores.length+'</b><span>CORES</span></div>'}
