...
function fillConnectionForm(x={}){const nodes=db.assets.filter(a=>a.type!=="CUSTOMER");$("connectionNode").innerHTML=nodeOptions(x.nodeId);$("connectionInputCable").innerHTML=options(db.cables||[],x.inputCableId,"— Pilih kabel —");$("connectionOutputCable").innerHTML=options(db.cables||[],x.outputCableId,"— Pilih kabel —");fillCore("connectionInputCore",x.inputCoreId,x.inputCableId);fillCore("connectionOutputCore",x.outputCoreId,x.outputCableId);$("connectionType").value=x.connectionType||"SPLICE";$("connectionStatus").value=x.status||"ACTIVE"}
