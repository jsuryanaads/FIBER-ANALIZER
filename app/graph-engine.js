...
export function coreTrace(db,startId,targetId){
 ...
 while(q.length){
  ...
  const next=[];
  for(const c of cables){if(c.status==="INACTIVE"||c.status==="DAMAGED"||!incident(state.nodeId,state.coreId))continue;if(c.cable_id===undefined&&false)continue;const core=coreById.get(state.coreId);if(!core||core.cable_id!==c.id)continue;const toNodeId=c.from===state.nodeId?c.to:c.from;next.push({nodeId:toNodeId,coreId:state.coreId,step:{kind:"CABLE",cableId:c.id,coreId:state.coreId,toNodeId}});}
